...
import { MdComputer, MdCloudUpload, MdDelete, MdBackpack, MdAttachMoney } from 'react-icons/md'
import { categories } from '../utils/data';
import { Loader } from './Loader';
import { deleteObject, getDownloadURL, ref, uploadBytesResumable } from 'firebase/storage';
import { storage } from '../firebase.config';
import { getAllSoftwareItems, saveItem } from '../utils/firebaseFunctions';
...
export const CreateContainer = () => {
  ...
  const uploadImage = (e) => {
    ...
    uploadTask.on('state_changed', (snapshot) => {
      ...
      console.log(error);
      setFields(true);
      setMsg('Error al cargar: Intenta de Nuevo');
  ...
  const saveDetails = () => {
    ...
    try {
      ...
      if (!title || !model || !imageAsset || !price || !category) {
        setFields(true);
        ...
        setIsLoading(false);
        setFields(true);
        setMsg('Datos Subidos con Exito');
        clearData();
        setAlertStatus('success');
        setTimeout(() => {
          setFields(false);
  ...
  const clearData = () => {
    ...
    setModel("");
    setPrice("");
    setCategory("Seleccionar Categoria");
  };
  ...
  return (
    <div className='w-full min-h-screen flex items-center justify-center  '>
      <div className='w-[90%] md:[75%] border border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center gap-4 '>
        {
          fields && (
        ...
        <div className='w-full py-2 border-b border-gray-300 flex items-center gap-2'>
          <MdComputer className='text-xl text-gray-700' />
          <input
            type="text"
            required
            value={title}
        ...
        <div className='w-full flex flex-col md:flex-row items-center gap-3'>
          <div className='w-full py-2 border-b border-gray-300 flex items-center gap-2'>
            <MdBackpack className='text-gray-700 text-2xl' />
            <input type="text"
              required
              value={model}
